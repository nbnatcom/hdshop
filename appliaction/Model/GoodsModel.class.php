...
class GoodsModel extends SystemModel {
	...
	public function update(){
		//重组POST @ 李春林 2015-09-16 11:59:41
		
		//parse_str重组_spec_array不完整这里单独处理
		$arr = explode("&",urldecode($_POST['_spec_array']));
		$_spec_array = array();
		foreach ($arr as $key => $value) {
			preg_match('/.*\[(\d+)\].*\=(.*)/',$value,$res);
			$_spec_array[$res[1]][] = $res[2];
		}
		$_POST['_spec_array'] = $_spec_array;
		
		parse_str($_POST['_goods_sn'],$myAarray);
		$_POST['_goods_sn'] = current($myAarray);
		parse_str($_POST['_goods_barcode'],$myAarray);
		$_POST['_goods_barcode'] = current($myAarray);
		parse_str($_POST['_goods_number'],$myAarray);
		$_POST['_goods_number'] = current($myAarray);
		parse_str($_POST['_shop_price'],$myAarray);
		$_POST['_shop_price'] = current($myAarray);
		parse_str($_POST['_market_price'],$myAarray);
		$_POST['_market_price'] = current($myAarray);
		parse_str($_POST['_cost_price'],$myAarray);
		$_POST['_cost_price'] = current($myAarray);
		parse_str($_POST['_products_ids'],$myAarray);
		$_POST['_products_ids'] = current($myAarray);
