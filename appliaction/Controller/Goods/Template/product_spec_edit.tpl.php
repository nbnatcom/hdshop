...
<style>
    ...
    .operator:hover{text-decoration: underline;color: #000000;}
...
<div class="content">
    ...
    <div class="site">
        Haidao Board <a href="#">商品管理</a> > 规格列表 > 编辑规格
    </div>
                            ...
                            <form action="<?php echo U('edit') ?>" class="addform" method="post">
                                <dd>
                                    <ul class="web">
                                        <li>
                                            <strong>规格名称：</strong>
                                            <input type="text" class="text_input" name="name" placeholder='' datatype="s" value="<?php echo $info['name'];?>" /><span class="Validform_checktip " style="padding-left: 43px;">设置商品规格名称　如：颜色
                                            </span>
                                        </li>
                                        <li>
                                            <strong>状态：</strong>
                                            <input type="radio" name="status" value="1" <?php if ($info['status'] == 1): ?>
                                            	checked="checked"
                                            <?php endif ?> /> 显示 <input type="radio" name="status" value="0" <?php if ($info['status'] == 0): ?>
                                            	checked="checked"
                                            <?php endif ?> /> 禁用<span class="Validform_checktip" style="margin-left:214px;">设置商品规格是否显示，默认为显示</span>
                                        </li>
                                    </ul>
                                    <dl class="blue_table mt10">
                                        ...
                                        <dd>
                                                <table>
                                                    <tbody id='spec_box'>
                                                    <?php $value= explode(',', $info['value']);?>
                                                    <?php foreach ($value as $key => $vo): ?>
                                                    <tr class="td_c"><td  width="350" align="left" height="35">
                                                        <?php if ($info['type'] == 1): ?>
                                                            <input class="text_input spec_input" type="text" name="value[]" value="<?php echo $vo?>" datatype="s"  alt="填写规格值" />
                                                        <?php else: ?>
                                                        	<input type="hidden" name="value[]"  value="<?php echo $vo?>" /><img src="<?php echo IMG_PATH;?>admin/spec_uppic.png" style="padding:10px;float:left" class="upimg"><img src="<?php echo $vo?>" class="img_border" height="43px" />
                                                        <?php endif ?>
                                                    </td>
...
function getValHtml(dataValue) {
    if(dataValue == undefined)
        dataValue = '';
    return '<input class="text_input spec_input" type="text" name="value[]" value="'+(dataValue?dataValue:"")+'"  alt="填写规格值" />';
}
//按钮(点击绑定)
