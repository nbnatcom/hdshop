		...
		$(function() {
			...
			var demo = $(".goodsForm").Validform({
				...
				beforeSubmit:function(curform){
					/*
					 * 将商品规格属于序列化后传递 避免提交input超限
					 * 2015-09-16 16:31:11 @李春林
					 */
					var _goods_barcode = $('[name^="_goods_barcode"]').serialize();
					$('[name^="_goods_barcode"]').remove();
					$(curform).append('<input type="hidden" name="_goods_barcode" value="'+_goods_barcode+'">');
					
					var _goods_sn = $('[name^="_goods_sn"]').serialize();
					$('[name^="_goods_sn"]').remove();
					$(curform).append('<input type="hidden" name="_goods_sn" value="'+_goods_sn+'">');
					
					var _spec_array = $('[name^="_spec_array"]').serialize();
					$('[name^="_spec_array"]').remove();
					$(curform).append('<input type="hidden" name="_spec_array" value="'+_spec_array+'">');
					
					var _goods_number = $('[name^="_goods_number"]').serialize();
					$('[name^="_goods_number"]').remove();
					$(curform).append('<input type="hidden" name="_goods_number" value="'+_goods_number+'">');
					
					var _shop_price = $('[name^="_shop_price"]').serialize();
					$('[name^="_shop_price"]').remove();
					$(curform).append('<input type="hidden" name="_shop_price" value="'+_shop_price+'">');
					
					var _market_price = $('[name^="_market_price"]').serialize();
					$('[name^="_market_price"]').remove();
					$(curform).append('<input type="hidden" name="_market_price" value="'+_market_price+'">');
					
					var _cost_price = $('[name^="_cost_price"]').serialize();
					$('[name^="_cost_price"]').remove();
					$(curform).append('<input type="hidden" name="_cost_price" value="'+_cost_price+'">');
					
					var _products_ids = $('[name^="_products_ids"]').serialize();
					$('[name^="_products_ids"]').remove();
					$(curform).append('<input type="hidden" name="_products_ids" value="'+_products_ids+'">');
					art.dialog({id:'tip',width: 320, title: false, cancel: false,fixed: true,lock: true, content:'正在保存数据,请勿刷新！'});
				},
