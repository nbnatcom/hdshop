...
<div class="content">
    <div class="site">
        Haidao Board <a href="#">会员管理</a> > 群发电子邮件
    </div>
    <span class="line_white"></span>
    <div class="goods mt10">
    	  <form class="addform" name="addform" action="<?php echo U('PushMail/lists'); ?>" method="post" autocomplete="off" submit="javascript:return false">
        <div class="vip_ss">
            <p>
                <strong>搜索会员：</strong>
        ...
        <div class="vip_tj mt10 clearfix">
            <strong>会员<br />统计</strong><b>共发放电子邮件数量：<font class="member_num">0</font></b><span>请选择发放优惠券的会员条件，系统将计算共发放优惠券数量</span>
        ...
        <div class="vip_fa mt10 clearfix">
        	<ul>
	        	<li>
		        	<strong>邮件标题：</strong>
		            <input class="input" name="title" datatype="*" value="{$user_name}" />
		            <p>&nbsp;&nbsp;填写发送的邮件标题，本功能需要先到全局设置配置短信</p>
	           	</li>
	           	<li>
		        	<strong>邮件内容：</strong>
		            <textarea class="input" name="content" datatype="*">{$user_name}</textarea>
		            <p>&nbsp;&nbsp;填写发送的邮件内容，本功能需要先到全局设置配置短信</p>
	            </li>
            </ul>
        </div>
        <div class="submit">
    ...
    <script>
        ...
        $(function() {
            ...
            $(".button_search").click(function() {
                ...
                $.get("<?php echo U('PushMail/search_user/?opt=search'); ?>", {"keyword": keyword.toString()}, function(data) {
                    if (data.status == 0) {
            ...
            function getmember(){
                ...
                var url = "<?php echo U('PushMail/search_user/?opt=search'); ?>";
